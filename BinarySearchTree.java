...
public class BinarySearchTree {
	...
	public int WordMax = 0;
	public int TempWordMax = 0;
   ...
   public int heightA(BTNode node) {
	    
	   int b = height(node);
	   return b - 1;
 	}
//    	  if ((t.numChildren(node) == 0)) { return 0;}
//    	  
//    	  else {
//    	  int h = 0;  // base case if the given node is external
//    	  
//    	  for (BTNode c : t.children(node))
//            {
//             h = Math.max(h, 1 + height(t,c));
//            }
//    	  return h;
//        }
    
//      public Iterable<BTNode> children(BTNode n)
//      {
//    	if (left(n)!= null)
...
//       public void children(BTNode node){
//    	   if (node == null)
//           {
//               return;
//           }
...
//       }
    ...
    public void maxSearchPath() {
    	
    	System.out.println(heightA(this.root));
    ...
    public void printTenMostCommonWords() {
    	for (int i =0; i <= 9; i++) {
    	printTenMostCommonWordsH(this.root);
    	System.out.println(StringMax + " " + WordMax);
    	resetNode(this.root, StringMax);
    	}
    }
    public void printTenMostCommonWordsH(BTNode node){
    	if (node == null){return;}
    	
    	printTenMostCommonWordsH(node.getLeft());
    	if(node.wordCounter > WordMax) {
    		WordMax = node.wordCounter;
    		StringMax = node.word;
    	}
    	printTenMostCommonWordsH(node.getRight());
    	

    	
    }
   
    public void resetNode (BTNode node, String replace){
		if (node.left != null) {
			resetNode(node.left, replace);
		}
		if (node.word.compareTo(replace) == 0) {
			node.wordCounter *= -1;
			WordMax = 0;
		}
		if (node.right != null) {
			resetNode(node.right, replace);
		}
